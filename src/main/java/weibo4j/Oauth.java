...
public class Oauth extends Weibo {
	...
	public void setRedirectUrl(String rl) {
		this.redirectUrl = rl;
	}
	...
	private String useRedirectUrl() {
		if (this.redirectUrl == null || this.redirectUrl.trim().equals("")) {
			return WeiboConfig.getValue("redirect_URI").trim();
		} else {
	...
	public AccessToken getAccessTokenByCode(String code) throws WeiboException {
		return new AccessToken(client.post(
				...
				new PostParameter[] {
						...
						new PostParameter("redirect_uri", this.useRedirectUrl()) },
				false, null));
	}
